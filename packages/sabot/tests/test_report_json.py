...
Run: pytest packages/sabot/tests/test_report_json.py
...
EXPORT_LINES = [
    ...
    {"id": "e.1.1", "issue_type": "task", "title": "harness", "status": "open",
     ...
     "labels": ["sab-harness", "sab-surface", "non-work"],
    ...
    {"id": "e.1.4", "issue_type": "task", "title": "unstamped finding", "status": "open",
     ...
     "labels": ["sab-finding", "sab-surface"],
     "dependencies": [{"depends_on_id": "e.1", "type": "parent-child"}]},
    # A harness whose own sab-harness label the agent dropped (only inherited
    # sab-surface remains). Metadata shape still identifies it; it must bucket as a
    ...
    {"id": "z.1", "issue_type": "task", "title": "other finding", "status": "open",
     "metadata": {"run_id": "run-B", "tier": "HIGH"}, "labels": ["sab-finding"]},
...
def test_mislabeled_harness_bucketed_by_metadata_and_flagged(stub_bd):
    """Bug 3: a harness that lost its sab-harness label (only inherited sab-surface)
