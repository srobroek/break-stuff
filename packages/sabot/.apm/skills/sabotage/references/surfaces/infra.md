...
|------|------|-------|-----------|---------|---------|
| trivy | default-on | local | `trivy fs --scanners misconfig,vuln,secret --format json .` | IaC misconfig, CVEs, secrets across Terraform, Docker, and k8s in one pass | broad; subsumes much of checkov for common rules |
...
| osv-scanner | default-on | global | `osv-scanner --format json -r .` | CVEs across every lockfile ecosystem | overlaps the repo's `dep-audit` package, which is preferred when present |
