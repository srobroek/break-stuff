...
Run: pytest packages/break-stuff/tests/test_build_ext_image.py
...
SCRIPT = (
    ...
    / ".apm" / "skills" / "break-stuff" / "scripts" / "build-ext-image.sh"
)
...
def dry_run(target: Path, base="break-stuff/rust:1", tag="break-stuff/rust-ext:1"):
    r = subprocess.run(
...
def test_froms_the_given_base(tmp_path):
    ...
    df = dry_run(tmp_path, base="break-stuff/rust:1")
    assert df.splitlines()[0] == "FROM break-stuff/rust:1"
...
def test_build_invokes_runtime_with_tag(tmp_path, stub_docker):
    ...
    r = subprocess.run(
        ["bash", str(SCRIPT), "--target", str(repo),
         "--base", "break-stuff/base:1", "--tag", "break-stuff/base-ext:1"],
        capture_output=True, text=True, env=env,
    )
    assert r.returncode == 0, r.stderr
    argv = rec.read_text()
    assert "build" in argv
    assert "-t break-stuff/base-ext:1" in argv
