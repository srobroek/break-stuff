...
#     --base     the surface image to extend, e.g. break-stuff/rust:1
#     --tag      the output image tag, e.g. break-stuff/rust-ext:1
...
[ -n "$BASE" ]   || { echo "build-ext-image: --base required (a break-stuff/<surface> image)" >&2; exit 2; }
[ -n "$TAG" ]    || { echo "build-ext-image: --tag required (the output image, e.g. break-stuff/rust-ext:1)" >&2; exit 2; }
