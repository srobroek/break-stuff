...
| semgrep | opt-in | local | `uvx semgrep --config p/command-injection --json build.rs setup.py` | a build script shelling out on a value it fetched or read | registry pack, code side only |
...
| osv-scanner | default-on | global | `osv-scanner --format json -r .` | a known-malicious build-dependency | shared with `infra.md`; prefer the `dep-audit` package |
