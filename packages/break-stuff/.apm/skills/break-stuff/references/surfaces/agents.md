...
| semgrep (ToB pack) | opt-in | local | `mise x ubi:opengrep/opengrep -- opengrep --config p/trailofbits --json <files>` | agent-adjacent code patterns the shipped ruleset misses | fetches the pack over the network |
