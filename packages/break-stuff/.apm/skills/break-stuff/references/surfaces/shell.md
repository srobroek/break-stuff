...
| semgrep | default-on | local | `mise x ubi:opengrep/opengrep -- opengrep --config p/bash --config p/command-injection --json <files>` | `eval` on interpolated data, `curl \| sh`, unsafe `mktemp` | registry packs; complements shellcheck with taint-shaped rules |
