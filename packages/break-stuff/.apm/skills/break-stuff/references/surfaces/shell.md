...
| semgrep | default-on | local | `uvx semgrep --config p/bash --config p/command-injection --json <files>` | `eval` on interpolated data, `curl \| sh`, unsafe `mktemp` | registry packs; complements shellcheck with taint-shaped rules |
